...
export async function POST(req) {
  try {
    ...
    const sheetPayload = {
      ...
      name,
      email,
      form_type: "hosur_brochure",
      timestamp: body?.timestamp || new Date().toISOString(),
    };

    const sheetResponse = await fetch(process.env.SHEET_URL6, {
      method: "POST",
    ...
    const parkName = "KSH Hosur Park I";
    const brochureLink =
      "https://www.kshinfra.com/brochure/KSH-Hosur-Park-1-Brochure.pdf";
...
Timestamp: ${sheetPayload.timestamp}`,
    ...
    console.error("Hosur brochure API error:", error);
    return Response.json(
      {
        success: false,
